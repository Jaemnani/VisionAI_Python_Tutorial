...
class ArrowKeypointsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return img, ang_label, kps, heatmap
...
class ArrowMultiNet(nn.Module):
    def __init__(self):
        super(ArrowMultiNet, self).__init__()
        self.conv1 = nn.Conv2d(1, 16, kernel_size=3, padding=1)
        self.bn1 = nn.BatchNorm2d(16)
...
class DeeperArrowMultiNet(nn.Module):
    def __init__(self):
        super(DeeperArrowMultiNet, self).__init__()
        # 첫 번째 블록: (1 -> 16 -> 16), 풀링
        self.block1 = nn.Sequential(
            ConvBNReLU(1, 16),      # (N,1,H,W) -> (N,16,H,W)
            ConvBNReLU(16, 16),     # (N,16,H,W) -> (N,16,H,W)
...
dataset = ArrowKeypointsDataset(image_dir='./images/', angle_dir='./directions/',  label_dir='./keypoints/', transform=transform)
...
for epoch in range(num_epochs):  # 여기서는 10 에포크로 설정
    ...
    with torch.no_grad():  # 그래디언트 계산 비활성화
        for images, angle_label, kps, heatmap_label in valid_loader:
            images = images.to(device)
...
def post_processing(output):
    ...
    sin_val, cos_val = output_ang[0].cpu().detach().numpy()
...
for i in range(test_cnt):
    ...
    print("KPS gt->pred : sp(%.2f, %.2f)->(%.2f, %.2f)(%.2f), ep(%.2f, %.2f)->(%.2f, %.2f)(%.2f)"%
          (ground_truth_kps[0][0], ground_truth_kps[0][1], heatmap_result[0][0], heatmap_result[0][1], result_diff[0], 
